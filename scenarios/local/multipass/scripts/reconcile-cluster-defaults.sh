...
if ! printf '%s\n' "${default_scs}" | grep -q "^${preferred_longhorn_sc}|true$"; then
  ...
  mp_exec "${SERVER_NAME}" "sudo k3s kubectl patch storageclass ${preferred_longhorn_sc} -p '{\"metadata\":{\"annotations\":{\"storageclass.kubernetes.io/is-default-class\":\"true\"}}}'"
fi
...
if [[ "${preferred_longhorn_sc}" != "longhorn" ]] && printf '%s\n' "${default_scs}" | grep -q '^longhorn|true$'; then
  ...
  mp_exec "${SERVER_NAME}" "sudo k3s kubectl patch storageclass longhorn -p '{\"metadata\":{\"annotations\":{\"storageclass.kubernetes.io/is-default-class\":\"false\"}}}'"
fi

if printf '%s\n' "${default_scs}" | grep -q '^longhorn-static|true$'; then
  log "Marking longhorn-static as non-default StorageClass"
  mp_exec "${SERVER_NAME}" "sudo k3s kubectl patch storageclass longhorn-static -p '{\"metadata\":{\"annotations\":{\"storageclass.kubernetes.io/is-default-class\":\"false\"}}}'"
fi
